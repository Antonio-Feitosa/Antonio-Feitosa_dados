...
lista.de.pacotes = c("tidyverse","lubridate","janitor", "magrittr", 
    "readr", "readxl","stringr","repmis","survey","srvyr") 
...
library(magrittr)
```
## Incluir e/ou carregar pacotes `readr`, `readxl`, `haven` e `DBI`
## "magrittr", "scales"
...
C:\R\Repositorio\dados

```{r}
processos <- read_rds("C://R/Repositorio/dados/processos_nested.rds")
```
...
```{r echo=FALSE}
bancadas <- read_rds("C://R/Repositorio/dados/bancadas.rds")
bancadas %>% head

```

```{r echo=FALSE}
coligacoes <- read_excel("C://R/Repositorio/dados/coligacoes.xlsx")
coligacoes %>% head
...
```{r echo=FALSE}
governismo_temer <- read_excel("C://R/Repositorio/dados/governismo_temer.xlsx")
governismo_temer %>% head
...
out <- apistrat %>%
  ...
  summarize(api_diff = weighted.mean(api00 - api99, pw), n = n())
...
- Qual a proporção de escolas por nível (Fundamental, Médio)? (use a variável nova que você criou e a função `survey_mean`), 
 ...
 -- Qual o coeficiente de variação (cv) dessa proporção?
...
out %>% 
  ggplot(aes(x = nivel, y = api_diff, fill = nivel,color=nivel,                        ymax = api_diff_upp, ymin = api_diff_low)) +
...
- a cada alteração de `index.rmd` e `index.html`, merge de master para gh-pages OU SIMPLESMENTE apague o branch e recrie o gh-pages
